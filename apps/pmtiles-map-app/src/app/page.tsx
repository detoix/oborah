...
export default function Home() {
  ...
  const activeSelectedId = draftPlacement ? draftPlacement.id : selectedBuildingId;

  const enterEditModeForItem = useCallback((item: CatalogItem) => {
  ...
  const updateDraftPosition = useCallback((position: { lng: number; lat: number }) => {
  ...
  const acceptDraft = useCallback(() => {
    ...
    addBuilding(draftPlacement.type, draftPlacement.position, draftPlacement.rotationY);
    exitDraftMode();
  }, [addBuilding, draftPlacement, exitDraftMode]);

  const handleDragOver = useCallback((e: React.DragEvent) => {
    if (e.dataTransfer.types.includes("application/obora-item")) {
      e.preventDefault();
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent) => {
      e.preventDefault();
      const data = e.dataTransfer.getData("application/obora-item");
                  ...
                  <button
                    ...
                    onClick={() =>
                      setDraftPlacement((draft) =>
                        draft
                          ? { ...draft, rotationY: draft.rotationY + Math.PI / 4 }
                          : draft,
                      )
                    }
